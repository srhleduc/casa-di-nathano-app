...
export default function ServiceTypeScreen({ onSelect, options, question }) {
  const opts = options || DEFAULT_OPTIONS;
  return (
    <div className="flex-1 flex flex-col items-center justify-center px-8 text-center">
      <h2 className="display-font text-4xl font-semibold mb-10">{question || "Sur place ou à emporter ?"}</h2>
      <div className={`flex flex-col md:flex-row gap-5 w-full ${opts.length > 2 ? "max-w-3xl" : "max-w-xl"}`}>
        {opts.map((o) => (
          <button
            key={o.value}
