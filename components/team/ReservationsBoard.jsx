...
export default function ReservationsBoard({ onTakeOrder = null } = {}) {
  ...
  // affichée) : une table signalée dans une AUTRE zone n'y serait pas vue,
  // d'où flaggedZoneIds ci-dessous — pour que le point apparaisse aussi sur
  // le bouton de zone correspondant, en permanence, quelle que soit la zone
  // affichée.
  ...
  const flaggedZoneIds = useMemo(() => {
    const s = new Set();
    for (const tid of flaggedTableIds) {
      const t = tables.find((x) => x.id === tid);
      if (t?.layoutId) s.add(t.layoutId);
    }
    return s;
  }, [flaggedTableIds, tables]);
  ...
  async function submitAdd() {
    ...
    if (af.slotMin == null) return setAddErr("Choisissez un créneau.");
    setAddBusy(true);
    setAddErr(null);
    try {
      ...
      console.error(e);
      setAddErr("La réservation n'a pas pu être enregistrée. Réessayez.");
    } finally {
      setAddBusy(false);
    }
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-4">
      <div className="flex items-center gap-3 mb-4">
        ...
        {layouts.length > 1 && (
          // Barre de zones permanente (pas un sélecteur qui repli tout) —
          // point rose sur le bouton d'une zone qui a un ajout /sat non
          // pointé, même quand on regarde une autre zone (PlanView ne
          // dessine le point que pour ses propres tables).
          <div className="flex flex-wrap gap-1.5">
            {layouts.map((l) => {
              const active = l.id === layoutId;
              const flagged = flaggedZoneIds.has(l.id);
              return (
                <button
                  key={l.id}
                  onClick={() => setLayoutId(l.id)}
                  className="tap-scale relative rounded-full px-3 py-1.5 text-sm font-bold border-2"
                  style={
                    active
                      ? { borderColor: "#e8622c", background: "#2c1c14", color: "#f5ebdd" }
                      : { borderColor: "#3a2b1f", color: "#a88f78" }
                  }
                >
                  {l.name}
                  ...
                  {flagged && (
                    <span
                      className="absolute rounded-full"
                      style={{ top: -3, right: -3, width: 10, height: 10, background: "#ff2d95", boxShadow: "0 0 6px #ff2d95" }}
                      aria-label="Ajout client /sat non pointé dans cette zone"
                    />
                  )}
                </button>
              );
        ...
        {services.map((s) => {
          ...
          return (
            <div
              ...
              className="tap-scale cursor-pointer rounded-xl p-3 pr-9 min-w-[180px] relative"
              style={{
            ...
            >
              <button
                ...
                title="Ajouter une réservation dans ce service"
                className="tap-scale absolute top-1.5 right-1.5 w-6 h-6 rounded-full text-sm font-bold flex items-center justify-center"
      ...
      {/* --- Ajout rapide d'une réservation --- */}
      {addForm && (
        <div
        ...
        >
          ...
          >
            ...
            <div className="flex gap-3 mb-3">
              <label className="flex-1 text-xs text-[#a88f78]">
                Personnes
                <input
                  type="number"
                  min={1}
              ...
              <label className="flex-1 text-xs text-[#a88f78]">
                ...
                <select
                  value={af.slotMin ?? ""}
                  onChange={(e) => setAf((x) => ({ ...x, slotMin: Number(e.target.value) }))}
                  className="w-full rounded-lg px-3 py-2 mt-1 text-sm"
                  style={inputStyle}
                >
                  {addSlots.map((sl) => (
                    <option key={sl.startMin} value={sl.startMin}>
                      {hhmm(sl.startMin)}
                    </option>
                  ))}
                </select>
