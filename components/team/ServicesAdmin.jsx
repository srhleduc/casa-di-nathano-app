...
import TakeawayHoursAdmin from "./TakeawayHoursAdmin";
...
export default function ServicesAdmin() {
  const restaurantFilter = useRestaurantFilter(); // non-null uniquement vu depuis l'espace Direction
  ...
  const updateSettings = (patch) => updateReservationSettings(patch, restaurantFilter);
      ...
      <div className="rounded-xl border border-[#3a2b1f] bg-[#211712] p-4 mb-4">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm">
          <label className="flex items-center gap-2 text-xs text-[#a88f78]">
            ...
            <NumInput value={settings.bookingLeadMinutes} min={0} onCommit={(v) => updateSettings({ bookingLeadMinutes: v ?? 30 }).catch((e) => console.error(e))} />
          </label>
          <label className="flex items-center gap-2 text-xs text-[#a88f78]">
            Marge de sécurité (min)
            <NumInput value={settings.safetyMarginMinutes} min={0} onCommit={(v) => updateSettings({ safetyMarginMinutes: v ?? 0 }).catch((e) => console.error(e))} />
          </label>
          <label className="flex items-center gap-2 text-xs text-[#a88f78]">
            Pas des créneaux (min)
            <NumInput value={settings.slotGranularityMinutes} min={5} onCommit={(v) => updateSettings({ slotGranularityMinutes: v ?? 15 }).catch((e) => console.error(e))} />
          </label>
          <button
            onClick={() => updateSettings({ onlineBookingEnabled: !settings.onlineBookingEnabled }).catch((e) => console.error(e))}
        ...
        <div className="mt-4 pt-3 border-t border-[#3a2b1f]">
          ...
          <div className="flex flex-wrap items-center gap-4 text-sm">
            <label className="flex items-center gap-2 text-xs text-[#a88f78]">
              1-2 pers. (min)
              <NumInput value={settings.durationMin1To2} min={0} onCommit={(v) => updateSettings({ durationMin1To2: v ?? 75 }).catch((e) => console.error(e))} />
            </label>
            <label className="flex items-center gap-2 text-xs text-[#a88f78]">
              3-4 pers. (min)
              <NumInput value={settings.durationMin3To4} min={0} onCommit={(v) => updateSettings({ durationMin3To4: v ?? 90 }).catch((e) => console.error(e))} />
            </label>
            <label className="flex items-center gap-2 text-xs text-[#a88f78]">
              5-6 pers. (min)
              <NumInput value={settings.durationMin5To6} min={0} onCommit={(v) => updateSettings({ durationMin5To6: v ?? 105 }).catch((e) => console.error(e))} />
            </label>
            <label className="flex items-center gap-2 text-xs text-[#a88f78]">
              + par personne au-delà de 6 (min)
              <NumInput value={settings.durationMinExtraPerPerson} min={0} onCommit={(v) => updateSettings({ durationMinExtraPerPerson: v ?? 10 }).catch((e) => console.error(e))} />
            </label>
          </div>
        ...
        <div className="mt-4 pt-3 border-t border-[#3a2b1f]">
          <div className="flex flex-wrap items-center gap-3 text-xs text-[#a88f78]">
            ...
            <TimeInput value={settings.earliestServiceTime} onCommit={(v) => updateSettings({ earliestServiceTime: v }).catch((e) => console.error(e))} />
            à
            <TimeInput value={settings.latestServiceTime} onCommit={(v) => updateSettings({ latestServiceTime: v }).catch((e) => console.error(e))} />
          </div>
          <div className="text-xs text-[#5a4a3a] mt-1 max-w-2xl">
      ...
      <div className="rounded-xl border border-[#3a2b1f] bg-[#211712] p-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          {dateOverrides.length > 0 && (
            <button onClick={() => deleteServiceOverridesForDate(date, restaurantFilter).catch((e) => console.error(e))} className="tap-scale text-xs text-red-400 font-bold">
        ...
        {dateOverrides.filter((o) => !o.isActive).map((o) => {
          ...
          return (
            <button
              key={o.id}
              onClick={() => upsertServiceOverride({ date, serviceNumber: o.serviceNumber, startTime: o.startTime, endTime: o.endTime, maxCovers: o.maxCovers, maxCoversByLayout: o.maxCoversByLayout || {}, isActive: true, restaurantId: restaurantFilter }).catch((e) => console.error(e))}
