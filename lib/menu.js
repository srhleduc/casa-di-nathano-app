...
// Groupes de parfums partagés (les 3 tailles de glace partagent GLACE_FLAVORS,
// "Sirop à l'eau"/"Diabolo" partagent SIROP_FLAVORS) — sert à la gestion des
// ruptures PAR PARFUM depuis l'onglet Ruptures.
export const FLAVOR_GROUPS = {
  glace: { label: "Glaces", flavors: GLACE_FLAVORS },
  sirop: { label: "Sirops", flavors: SIROP_FLAVORS },
};
export function flavorGroupFor(name) {
  const cfg = flavorConfigFor(name);
  if (!cfg) return null;
  if (cfg.flavors === GLACE_FLAVORS) return "glace";
  if (cfg.flavors === SIROP_FLAVORS) return "sirop";
  return null;
}
// Clé stockée dans `ruptures.item_id` pour un parfum indisponible.
export const flavorRuptureKey = (groupKey, flavor) => `flavor:${groupKey}:${flavor}`;
export function parseFlavorRuptureKey(key) {
  const m = /^flavor:([^:]+):(.+)$/.exec(key || "");
  return m ? { group: m[1], flavor: m[2] } : null;
}

// Liste de parfums d'un groupe : la version éditée en base (useFlavors) si elle
// existe, sinon la liste statique par défaut. `liveByGroup` = { glace, sirop }.
export function flavorsForGroup(liveByGroup, groupKey) {
  const live = liveByGroup && liveByGroup[groupKey];
  if (Array.isArray(live) && live.length) return live;
  return FLAVOR_GROUPS[groupKey]?.flavors || [];
}
