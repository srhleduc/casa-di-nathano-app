...
const FALLBACK_RESTAURANT = { id: RESTAURANT_ID, name: "Casa Di Nathano", city: "", phone: "" };
...
export function useRestaurant(explicitId) {
  ...
  const [restaurant, setRestaurant] = useState(id === RESTAURANT_ID ? FALLBACK_RESTAURANT : { id, name: "", city: "", phone: "" });
  ...
  useEffect(() => {
    ...
    supabase
      .from("restaurants")
      .select("id, name, city, accent_color, phone")
      .eq("id", id)
      .maybeSingle()
      .then(({ data }) => {
        if (!cancelled && data) {
          setRestaurant({ id: data.id, name: data.name, city: data.city, accentColor: data.accent_color, phone: data.phone });
        }
      });
    return () => {
      cancelled = true;
...
export function useRestaurantsList() {
  ...
  useEffect(() => {
    ...
    supabase
      .from("restaurants")
      .select("id, name, city, accent_color, google_review_url")
      .order("name")
      .then(({ data }) => {
        if (!cancelled && data) {
          setRestaurants(
            data.map((r) => ({ id: r.id, name: r.name, city: r.city, accentColor: r.accent_color, googleReviewUrl: r.google_review_url }))
          );
        }
      });
    return () => {
