...
// --- grille 70 cm (1 case = 1 table) : détail interne en 35 cm ---
{
  const empty4 = () => Array.from({ length: 4 }, () => Array(4).fill("empty"));
  const L70 = { gridRows: 4, gridCols: 4, cellSizeCm: 70, cells: empty4() };
  const wall = [0, 1, 2, 3].map((c) => ({ id: "w" + c, gridRow: 2, gridCol: c }));
  const cc = [{ id: "c1", name: "WC", endpointA: { row: 0, col: 0 }, endpointB: { row: 3, col: 3 }, minWidthCells: 2, priority: "obligatoire" }];
  ok(!evaluateConstraints(L70, wall, cc).valid, "70 cm : rangée de tables coupe le passage obligatoire");
  ok(evaluateConstraints(L70, [], cc).valid, "70 cm : sans tables, passage obligatoire libre");

  // table sur une case dont une demi-case est une porte → on-door
  const cells = empty4();
  cells[1][1] = { s: "v", a: "D", b: "empty" };
  const Ldoor = { gridRows: 4, gridCols: 4, cellSizeCm: 70, cells };
  const r = evaluateConstraints(Ldoor, [{ id: "t", gridRow: 1, gridCol: 1 }], []);
  ok(!r.valid && r.violations.some((v) => v.type === "on-door"), "70 cm : table sur une demi-case porte → on-door");
}
