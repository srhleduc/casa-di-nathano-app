...
//
// `occupiedWithoutReservationCovers` / `occupiedByLayout` (option, service en
// cours uniquement) : couverts de tables actuellement occupées par une
// commande SANS réservation qui les tienne (clients de passage — /sat, ou
// avant qu'une réservation « Passage » ne soit posée). Ils ne comptent dans
// aucune réservation mais consomment bien de la capacité : `remaining`/`full`
// (globaux et par zone) les prennent en compte, en plus de `reserved`, et
// sont renvoyés à part dans `occupied` pour que l'équipe voie la différence
// entre réservé et occupé.
export function serviceSynthesis(
  reservationsForService,
  service,
  unassignedCount = 0,
  occupiedWithoutReservationCovers = 0,
  occupiedByLayout = {}
) {
  ...
  const occupied = occupiedWithoutReservationCovers || 0;
  const consumed = reserved + occupied;
  const capacity = service && service.maxCovers != null ? service.maxCovers : null;
  const remaining = capacity != null ? Math.max(0, capacity - consumed) : null;
  ...
  if (Object.keys(byLayout).length > 0) {
    ...
    zones = Object.entries(byLayout).map(([layoutId, cap]) => {
      ...
      const occ = (occupiedByLayout && occupiedByLayout[layoutId]) || 0;
      const cons = res + occ;
      return { layoutId, reserved: res, occupied: occ, capacity: c, remaining: Math.max(0, c - cons), full: c > 0 && cons >= c };
    });
  }
  ...
  return {
    ...
    remaining,
    unassignedCount,
    full: capacity != null && consumed >= capacity,
    zones,
  };
}
