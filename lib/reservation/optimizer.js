...
function seatingOptionsFor(partySize, tables, combinations) {
  ...
  for (const t of tables) {
    if (t.active === false || t.blocked === true) continue;
    if (tableCapMax(t) >= partySize) {
      opts.push({
        ...
        priorityOrder: t.priorityOrder == null ? null : Number(t.priorityOrder),
        penalty: 0,
      });
    }
  }
...
export function solve(input) {
  ...
  for (const r of reservations) {
    optionsByRes[r.id] = seatingOptionsFor(r.partySize, tables, combinations)
      ...
      // À score égal, l'ordre de priorité de remplissage départage (plus petit
      // priorityOrder = rempli en premier). Simple bris d'égalité : n'altère
      // ni l'acceptation ni le calcul de gaspillage.
      .sort((a, b) => b._score - a._score || (a.priorityOrder ?? Infinity) - (b.priorityOrder ?? Infinity));
  }
