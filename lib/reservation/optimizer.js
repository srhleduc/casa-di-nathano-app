...
// Pénalité d'une combinaison marquée « exceptionnelle » (is_usual = false),
// indépendante de penalty_score. Assez grande pour qu'une combinaison
// habituelle de fit comparable passe toujours devant, assez petite pour qu'une
// exceptionnelle reste ACCEPTÉE quand c'est la seule option (ACCEPT_BONUS
// domine). penalty_score reste le réglage fin à l'intérieur de chaque statut.
const EXCEPTIONAL_COMBO_PENALTY = 200;
// idx de base des combinaisons : après toutes les tables (on préfère toujours
// une table seule), puis dans l'ordre où le client les envoie (déjà trié
// habituelle → pénalité → clé stable).
const COMBO_IDX_BASE = 1e6;
...
// `preferredLayoutId` (zone choisie par le client sur /reserver) : quand il est
// défini, on EXCLUT toute option dont une table n'est pas dans cette zone
// (restriction dure — le choix de zone est obligatoire côté client).
function seatingOptionsFor(partySize, tables, combinations, { preferredLayoutId = null, tableLayoutById = {} } = {}) {
  const opts = [];
  const inZone = (tid) => preferredLayoutId == null || tableLayoutById[tid] === preferredLayoutId;
  ...
  (combinations || []).forEach((c, ci) => {
    if ((c.capacity ?? 0) >= partySize && Array.isArray(c.tableIds) && c.tableIds.length) {
      ...
      opts.push({
        ...
        idx: COMBO_IDX_BASE + ci,
        // « exceptionnelle » (is_usual === false) → forte pénalité en plus du
        // penalty_score → une habituelle disponible passe toujours devant.
        penalty: (Number(c.penaltyScore) || 0) + (c.isUsual === false ? EXCEPTIONAL_COMBO_PENALTY : 0),
        comboId: c.id,
      });
    }
  });
...
// input : { tables:[{id,active,blocked,layoutId,capacityMin,capacityPreferred,
//             capacityMax,capacityBase?,priorityOrder}], combinations,
//           reservations:[{id,partySize,startMin,durationMin,preferredLayoutId?}],
//           safetyMarginMinutes }
