...
//
// La grille visible a une case = une table (70 cm). evaluateConstraints la
// détaille en interne en 35 cm (expandTo35) pour valider des largeurs de
// passage précises, en tenant compte des éventuelles demi-cases.

import { expandTo35 } from "./grid.js";
...
export function evaluateConstraints(layoutIn, placedTables, circulationConstraintsIn = [], { combinations = [] } = {}) {
  const violations = [];
  const notes = [];
  let penalty = 0;

  // Détail en grille uniforme 35 cm : tout le reste (footprints, chemins,
  // largeurs) raisonne à cette échelle.
  const layout = expandTo35(layoutIn);
  const scale = layout.scale || 1;
  const placed = (placedTables || [])
    .filter((t) => t.gridRow != null && t.gridCol != null)
    .map((t) => ({ ...t, gridRow: t.gridRow * scale, gridCol: t.gridCol * scale }));
  const circulationConstraints = (circulationConstraintsIn || []).map((cc) => {
    const a = cc.endpointA || cc.endpoint_a || {};
    const b = cc.endpointB || cc.endpoint_b || {};
    return {
      ...cc,
      endpointA: { row: (a.row ?? a.r ?? 0) * scale, col: (a.col ?? a.c ?? 0) * scale },
      endpointB: { row: (b.row ?? b.r ?? 0) * scale, col: (b.col ?? b.c ?? 0) * scale },
    };
  });
