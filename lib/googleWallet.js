  ...
  try {
    ...
    const res = await fetchWithTimeout(
      `${WALLET_API}/loyaltyObject/${encodeURIComponent(objectId)}/addMessage`,
      {
        ...
        body: JSON.stringify({
          message: { id, header, body, messageType: "TEXT" },
        }),
      }
    );
    if (res.ok) {
      console.log(`[wallet] addMessage ${objectId} id=${id} → 200 OK`);
      return "ok";
    }
    if (res.status === 404) {
    ...
    console.error(
      `[wallet] addMessage ${objectId} id=${id} → ${res.status}: ${await safeText(res)}`
    );
    return "error";
  } catch (err) {
