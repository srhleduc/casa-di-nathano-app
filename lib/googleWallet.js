...
// Seul TEXT_AND_NOTIFY déclenche une vraie notification push. TEXT n'ajoute
// que le message au dos de la carte (indicateur "nouveau message", pas d'alerte).
// Google limite les push à 3 par pass / 24 h (au-delà : 429 / QuotaExceeded) —
// géré dans addWalletMessage (log + statut 'quota', jamais bloquant).
const MESSAGE_TYPE = "TEXT_AND_NOTIFY";
...
// Ajoute un message à l'objet fidélité du client, AVEC notification push
// (messageType TEXT_AND_NOTIFY). Réutilisable pour les deux cas (changement de
// solde / déblocage de bon).
// Renvoie : 'ok' | 'skipped' (client sans Wallet) | 'quota' (limite 3 push /
// 24 h atteinte, message non notifié) | 'error'. Ne lève jamais : l'appelant
// est fire-and-forget.
//
// `messageId` : identifiant déterministe et unique du message. Google
// déduplique et NE PUSH PAS si le message n'est pas "nouveau" ; sans id
// explicite, deux "Nouveau solde : X" se ressemblent trop et le push est
// avalé. On passe donc un id stable par (client + évènement métier) — cf.
// syncWalletAfterPointsChange. Sans id fourni : fallback horodaté (toujours
// unique, toujours présent).
export async function addWalletMessage(customerId, header, body, messageId) {
  const objectId = walletObjectId(customerId);
  const id = String(messageId || `${ISSUER_ID}.msg_${Date.now()}`).slice(0, 100);
  try {
    const token = await getIssuerAccessToken();
    const res = await fetchWithTimeout(
      `${WALLET_API}/loyaltyObject/${encodeURIComponent(objectId)}/addMessage`,
      {
        ...
        body: JSON.stringify({
          message: { id, header, body, messageType: MESSAGE_TYPE },
        }),
      }
    );
    if (res.ok) {
      console.log(`[wallet] addMessage ${objectId} id=${id} → 200 OK (${MESSAGE_TYPE})`);
      return "ok";
    }
    const errBody = await safeText(res);
    if (res.status === 404) {
      console.warn(`[wallet] addMessage ${objectId} → 404 (objet inexistant, client sans Wallet)`);
      return "skipped";
    }
    // Limite Google : 3 notifications push par pass / 24 h. Attendu pour un
    // client très actif — on log en info et on n'échoue pas.
    if (res.status === 429 || /quota|resource_exhausted|rate limit/i.test(errBody)) {
      console.warn(
        `[wallet] addMessage ${objectId} id=${id} → ${res.status} quota push atteint (3/24 h), message non notifié`
      );
      return "quota";
    }
    console.error(`[wallet] addMessage ${objectId} id=${id} → ${res.status}: ${errBody}`);
    return "error";
  } catch (err) {
    console.error(`[wallet] addWalletMessage ${objectId} id=${id}:`, err?.message || err);
    return "error";
  }
}
...
export async function syncWalletAfterPointsChange(customerId, newSolde, pointsAdded = 0) {
  ...
  if (patch === "skipped") {
    console.log(`[wallet] sync ${objectId}: patch=skipped (client sans Wallet), aucun message`);
    return { patch, message: "skipped" };
  }

  const solde = Number(newSolde);
  const prevSolde = solde - (Number(pointsAdded) || 0);
  const unlocked = rewardsUnlocked(prevSolde, solde);
  const key = safeId(customerId);

  let message = "none";
  if (unlocked > 0) {
    const milestone = Math.floor(solde / POINTS_PER_REWARD);
    const bons = unlocked === 1 ? "un bon de 5 €" : `${unlocked} bons de 5 €`;
    console.log(`[wallet] sync ${objectId}: ${prevSolde}->${solde}, ${unlocked} bon(s) débloqué(s) (palier ${milestone})`);
    message = await addWalletMessage(
      customerId,
      PROGRAM_HEADER,
      `Bravo ! Vous avez débloqué ${bons}`,
      `${ISSUER_ID}.${key}.bon.${milestone}`
    );
  } else if (solde !== prevSolde) {
    console.log(`[wallet] sync ${objectId}: ${prevSolde}->${solde}, message "nouveau solde"`);
    message = await addWalletMessage(
      customerId,
      PROGRAM_HEADER,
      `Nouveau solde : ${solde} points`,
      `${ISSUER_ID}.${key}.solde.${solde}`
    );
  } else {
    console.log(`[wallet] sync ${objectId}: solde inchangé (${solde}), aucun message`);
  }
