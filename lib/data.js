...
// Réactivation manuelle d'un bon (caisse/admin) — repasse actif avec une
// nouvelle échéance à J+21, cf. cahier des charges.
export async function reactivatePromoCode(id) {
  const { error } = await supabase
    .from("promo_codes")
    .update({ status: "actif", expires_at: new Date(Date.now() + 21 * 864e5).toISOString() })
    .eq("id", id);
  if (error) throw error;
}
