...
// Pointages depuis une borne locale donnée (recalculée à chaque reload),
// avec mise à jour realtime. Écrit à part de useRealtimeTable, qui ne
// supporte qu'un filtre eq simple et pas de borne de date en plus du
// restaurant. `getSince` renvoie un Date (minuit local du premier jour
// voulu) ; ex. aujourd'hui minuit, ou le lundi de la semaine en cours.
function usePointageEntriesSince(getSince) {
  ...
  const reload = useCallback(async () => {
    ...
    let query = supabase.from("pointage_entries").select("*").gte("occurred_at", since.toISOString());
    if (eq?.column) query = query.eq(eq.column, eq.value);
    const { data, error } = await query;
    if (requestId !== latestRequestId.current) return;
    if (!error && data) setEntries(data.map(mapPointageEntryRow));
    setLoading(false);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`realtime:pointage_entries_since:${instanceId}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "pointage_entries" }, () => {
        if (cancelled) return;
...
export function useTodayPointageEntries() {
  return usePointageEntriesSince(() => {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d;
  });
...
export async function fetchPointageEntriesForRange(restaurantId, startDate, endDate) {
  ...
  const { data, error } = await supabase
    .from("pointage_entries")
    .select("staff_id, type, occurred_at")
    .eq("restaurant_id", restaurantId)
