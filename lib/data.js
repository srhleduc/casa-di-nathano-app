...
function mapTableRow(row) {
  return {
    ...
    active: row.active,
    // Config réservation (Phase 3) — colonnes ajoutées par la migration
    // reservations.sql, nullables tant que la table n'est pas configurée.
    capacityBase: row.capacity_base ?? 2,
...
// table_plan). `cells` = tableau 2D de codes empty/S/P/T/D/W. Plusieurs plans
// nommés par restaurant.
...
export async function createRoomLayout(name, { gridRows = 12, gridCols = 12, cells } = {}) {
  const grid = cells || Array.from({ length: gridRows }, () => Array.from({ length: gridCols }, () => "empty"));
  const { data, error } = await supabase
    .from("room_layouts")
    .insert({ restaurant_id: RESTAURANT_ID, name: String(name).trim(), grid_rows: gridRows, grid_cols: gridCols, cells: grid })
    .select("id")
    .single();
  if (error) throw error;
  return data.id;
...
export async function createServiceTemplate({ serviceNumber, label, startTime, endTime, maxCovers = null, activeByDefault = true }) {
...
export async function upsertServiceOverride({ date, serviceNumber, startTime, endTime, maxCovers = null, isActive = true, autoGenerated = false }) {
  const { error } = await supabase.from("service_overrides").upsert(
    ...
    { onConflict: "restaurant_id,date,service_number" }
  );
  if (error) throw error;
}
...
export async function createReservation({ customerName, customerPhone, partySize, requestedAt, estimatedDurationMinutes, source = "client", note = null }) {
  const { data, error } = await supabase
    .from("reservations")
    .insert({
      restaurant_id: RESTAURANT_ID,
