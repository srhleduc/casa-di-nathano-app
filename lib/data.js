...
function mapIngredientRow(row) {
  return { id: row.id, name: row.name, unit: row.unit, costPerUnit: Number(row.cost_per_unit) };
}
...
export async function insertIngredient({ name, unit, costPerUnit }) {
  const { error } = await supabase.from("ingredients").insert({ name, unit, cost_per_unit: costPerUnit });
  if (error) throw error;
}

export async function updateIngredient(id, { name, unit, costPerUnit }) {
  const row = {};
  if (name !== undefined) row.name = name;
