...
// Pointages du jour (heure locale), avec mise à jour realtime — pour le
// dashboard de présence. Écrit à part de useRealtimeTable, qui ne supporte
// qu'un filtre eq simple et pas de borne de date en plus du restaurant.
export function useTodayPointageEntries() {
  ...
  const reload = useCallback(async () => {
    ...
    let query = supabase.from("pointage_entries").select("*").gte("occurred_at", startOfDay.toISOString());
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`realtime:pointage_entries_today:${instanceId}`)
