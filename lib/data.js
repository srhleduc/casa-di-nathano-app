...
function mapTableRow(row) {
  return { id: row.id, number: row.number, active: row.active, createdAt: new Date(row.created_at).getTime() };
}
...
export async function addTable(number) {
  const { error } = await supabase.from("tables").insert({ restaurant_id: RESTAURANT_ID, number: String(number).trim() });
  if (error) throw error;
}
