...
export async function insertSlot({ label, capacity }) {
  const { error } = await supabase.from("slots").upsert({ restaurant_id: RESTAURANT_ID, label, capacity }, { onConflict: "label" });
  if (error) throw error;
}
...
export async function insertMenuItem({ id, name, price, cat, ingredients, photoUrl }) {
...
export async function updateMenuItem(id, { name, price, cat, ingredients, photoUrl }) {
  const row = {};
...
export function usePizzaStock() {
  const { rows, loading, reload } = useRealtimeTable({ table: "pizza_stock", mapRow: mapPizzaStockRow });
  return { pizzaStock: rows[0] || EMPTY_PIZZA_STOCK, loading, reload };
}

export async function setPizzaStock({ total, safetyMargin }) {
  const { error } = await supabase.from("pizza_stock").update({ total, safety_margin: safetyMargin }).eq("id", 1);
  if (error) throw error;
}
...
export function useTestMode() {
  const { rows, loading, reload } = useRealtimeTable({ table: "test_mode", mapRow: (r) => r.enabled });
  return { testMode: { enabled: rows[0] || false }, loading, reload };
}

export async function setTestModeEnabled(enabled) {
  const { error } = await supabase.from("test_mode").update({ enabled }).eq("id", 1);
  if (error) throw error;
}
