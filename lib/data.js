...
// Renvoie { glace: [...noms], sirop: [...noms] } trié par sort_order puis nom.
// Table absente/vide → tableaux vides ; les composants retombent alors sur les
// listes statiques de lib/menu.js (flavorsForGroup).

export function useFlavors() {
  const { rows, loading, reload } = useRealtimeTable({
    table: "menu_flavors",
    mapRow: (r) => ({ id: r.id, grp: r.grp, name: r.name, sortOrder: r.sort_order }),
  });
  const flavors = { glace: [], sirop: [] };
  [...rows]
    .sort((a, b) => a.sortOrder - b.sortOrder || a.name.localeCompare(b.name))
    .forEach((r) => {
      if (flavors[r.grp]) flavors[r.grp].push(r.name);
    });
  return { flavors, loading, reload };
}

export async function addFlavor(grp, name) {
  const { error } = await supabase.from("menu_flavors").insert({ grp, name: String(name).trim(), sort_order: 999 });
  if (error) throw error;
}

export async function removeFlavor(grp, name) {
  const { error } = await supabase.from("menu_flavors").delete().eq("grp", grp).eq("name", name);
  if (error) throw error;
}
