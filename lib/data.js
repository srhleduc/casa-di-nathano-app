...
import { legacyOptionGroupsForItem, PANUZZO_CUTOFF_HOUR } from "./menu";
import { estimateDurationMin } from "./reservation/slots";
import { servicesForDate, activeServiceGroups } from "./reservation/services";
...
// Lecture fraîche (pas le cache realtime) de la commande sur place ouverte
// non encaissée qui touche l'une de `tableIds` — utilisée en repli quand une
// insertion concurrente a déclenché l'index d'unicité (deux /sat au même
// instant). Renvoie { id, reservationId } ou null.
export async function fetchOpenDineInOrderForTables(tableIds) {
  ...
  const { data, error } = await supabase
    .from("orders")
    .select("id, table_ids, reservation_id, service_type, status, paid, scheduled_for")
    .eq("restaurant_id", RESTAURANT_ID)
    .overlaps("table_ids", ids)
    .neq("status", "servie");
  if (error) throw error;
  const hit = (data || []).find(
    (o) =>
      !isTakeawayLike(o.service_type) &&
      !o.paid &&
      (!o.scheduled_for || o.scheduled_for === todayStr)
  );
  return hit ? { id: hit.id, reservationId: hit.reservation_id || null } : null;
}
...
function mapMenuItemRow(row) {
  return {
    ...
    // "midi" | "soir" | null (null = disponible à tous les services).
    serviceRestriction: row.service_restriction || null,
  };
}
...
export async function insertMenuItem({ id, name, price, cat, ingredients, photoUrl, dineInOnly, takeawayOnly, staffOnly, featured, serviceRestriction }) {
  const { error } = await supabase.from("menu_items").insert({
    ...
    service_restriction: serviceRestriction ?? null,
  });
  if (error) throw error;
}

export async function updateMenuItem(id, { name, price, cat, ingredients, photoUrl, dineInOnly, takeawayOnly, staffOnly, featured, serviceRestriction }) {
  const row = {};
  ...
  if (serviceRestriction !== undefined) row.service_restriction = serviceRestriction;
...
export async function createServiceTemplate({ serviceNumber, label, startTime, endTime, maxCovers = null, activeByDefault = true, activeWeekdays, restaurantId }) {
...
export async function upsertServiceOverride({ date, serviceNumber, startTime, endTime, maxCovers = null, maxCoversByLayout, isActive = true, autoGenerated = false, restaurantId }) {
...
export async function deleteServiceOverridesForDate(date, restaurantId) {
  const { error } = await supabase.from("service_overrides").delete().eq("restaurant_id", restaurantId || RESTAURANT_ID).eq("date", date);
  if (error) throw error;
}
...
export async function createServiceException({ dateStart, dateEnd, serviceNumber = null, mode = "off", label = null, restaurantId }) {
...
// "services" de réservation (fenêtres de réservation, plus étroites), voir
// supabase/migrations_manual/takeaway_hours.sql. Une ligne par jour de
// semaine (0=dimanche…6=samedi), seedée à la création du restaurant ; les
// champs midi/soir sont indépendamment nullable (pas de service ce jour-là
// dans ce créneau).

function mapTakeawayHoursRow(row) {
  return {
    id: row.id,
    restaurantId: row.restaurant_id,
    weekday: row.weekday,
    midiOpen: hhmm(row.midi_open),
    midiClose: hhmm(row.midi_close),
    soirOpen: hhmm(row.soir_open),
    soirClose: hhmm(row.soir_close),
  };
}

export function useTakeawayHours() {
  const { rows, loading, reload } = useRealtimeTable({
    table: "takeaway_hours",
    mapRow: mapTakeawayHoursRow,
    orderColumn: "weekday",
    eq: useRestaurantEq(),
  });
  return { takeawayHours: rows, loading, reload };
}

export async function updateTakeawayHours(weekday, patch, restaurantId) {
  const row = {};
  if ("midiOpen" in patch) row.midi_open = patch.midiOpen || null;
  if ("midiClose" in patch) row.midi_close = patch.midiClose || null;
  if ("soirOpen" in patch) row.soir_open = patch.soirOpen || null;
  if ("soirClose" in patch) row.soir_close = patch.soirClose || null;
  const { error } = await supabase.from("takeaway_hours").update(row).eq("restaurant_id", restaurantId || RESTAURANT_ID).eq("weekday", weekday);
  if (error) throw error;
}

// Groupes de service ("midi" / "soir") actuellement OUVERTS pour ce
// restaurant, d'après les services de réservation configurés (écran
// Réservations > Services — horaires, jours de fermeture, exceptions).
// Sert à restreindre un produit du menu à un service (menu_items.
// service_restriction) dans les écrans de prise de commande. Recalculé
// chaque minute. Même coupure « midi/soir » que la formule (PANUZZO_CUTOFF_HOUR).
export function useActiveMenuServiceGroups() {
  ...
  return useMemo(() => {
    ...
    const services = servicesForDate(today, serviceTemplates, serviceOverrides, serviceExceptions);
    return activeServiceGroups(services, nowMin, PANUZZO_CUTOFF_HOUR * 60);
  }, [serviceTemplates, serviceOverrides, serviceExceptions, nowMin]);
}
...
function mapReservationSettingsRow(row) {
  return {
    ...
    // Durée d'occupation estimée par taille de groupe — voir
    // estimateDurationMin() dans lib/reservation/slots.js. Calée après un
    // service réel (phase 9), anciennes constantes en valeur par défaut.
    durationMin1To2: row.duration_min_1_2 ?? 75,
    durationMin3To4: row.duration_min_3_4 ?? 90,
    durationMin5To6: row.duration_min_5_6 ?? 105,
    durationMinExtraPerPerson: row.duration_min_extra_per_person ?? 10,
  };
}
...
export async function updateReservationSettings(patch, restaurantId) {
  const row = {};
  ...
  if (patch.durationMin1To2 != null) row.duration_min_1_2 = patch.durationMin1To2;
  if (patch.durationMin3To4 != null) row.duration_min_3_4 = patch.durationMin3To4;
  if (patch.durationMin5To6 != null) row.duration_min_5_6 = patch.durationMin5To6;
  if (patch.durationMinExtraPerPerson != null) row.duration_min_extra_per_person = patch.durationMinExtraPerPerson;
  const { error } = await supabase.from("reservation_settings").upsert(
    { restaurant_id: restaurantId || RESTAURANT_ID, ...row },
...
export async function createReservation({
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      ...(arrivedAt ? { arrived_at: arrivedAt } : {}),
    })
    .select("id")
    .single();
  if (error) throw error;
...
// Suppression définitive (pas un « Annuler » — statut conservé, juste hors
// vue) : client qui appelle en plein service pour décommander, croix rapide
// sur la carte, comme la suppression d'une commande. reservation_table_
// assignments suit en cascade (FK on delete cascade) ; une commande déjà
// liée (orders.reservation_id) est juste déliée (on delete set null).
...
// "YYYY-MM-DDTHH:MM:00" heure murale — même repère que requested_at.
function wallClock(d) {
  const p = (n) => String(n).padStart(2, "0");
  return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}T${p(d.getHours())}:${p(d.getMinutes())}:00`;
}

// Occupe une (ou plusieurs) table(s) sans commande : réservation « Passage »
// déjà installée (seated), forcée sur les tables. ≥2 tables = combinaison
// éphémère (statut « groupée » + trait). Se clôt à l'encaissement de la
// commande qui viendra s'y rattacher (boucle du board), ou à la main via
// « Libérer ». `tables` : le registre, pour estimer les couverts + la zone.
// Renvoie l'id de la réservation créée, ou null si aucune table.
export async function createWalkInReservationForTables(tableIds, { tables = [], durationSettings } = {}) {
  const ids = [...new Set(tableIds || [])].filter(Boolean);
  if (!ids.length) return null;
  const party =
    ids.reduce((s, tid) => {
  ...
  const layoutId = tables.find((x) => x.id === ids[0])?.layoutId || null;
  const now = new Date();
  const rid = await createReservation({
    customerName: "Passage",
    partySize: party,
    // Antidaté d'une minute : la table est « occupée » tout de suite, sans
    // passer par un bref « à venir ».
    requestedAt: wallClock(new Date(now.getTime() - 60000)),
    estimatedDurationMinutes: estimateDurationMin(party, durationSettings),
    source: "walk_in",
    status: "seated",
    arrivedAt: now.toISOString(),
    preferredLayoutId: layoutId,
  });
  await setReservationTables(rid, ids, { manual: true });
  return rid;
}
...
export async function awardLoyaltyPointsFromCaisse(phone, amount, orderId) {
  const { data, error } = await supabase.rpc("award_loyalty_points", {
    ...
    p_source: "caisse",
    // Établissement de CE passage — sert à savoir, si le compteur de
    // passages atteint 3 avec ce mouvement, où proposer l'avis Google.
...
// Lien d'avis Google par établissement — utilisé par le SMS "avis_google"
// (3e passage) pour proposer le bon lien selon le restaurant détecté.
export async function updateRestaurantGoogleReviewUrl(restaurantId, url) {
  const { error } = await supabase
    .from("restaurants")
...
// Son de notification "commande autonome" personnalisé par établissement
// (voir components/direction/NotificationSoundAdmin.jsx et lib/sound.js).
// null = revient au carillon par défaut généré en direct.
// Bucket "chime-sounds" (pas "notification-sounds") : certaines listes de
// filtrage des bloqueurs de pub/vie privée bloquent silencieusement toute
// URL contenant "notification" (ciblant à l'origine les popups
// d'autorisation web push), ce qui faisait échouer l'upload avec un simple
// "Failed to fetch" chez l'utilisatrice/utilisateur concerné.
export async function uploadNotificationSound(file) {
  const ext = file.name.split(".").pop() || "mp3";
  const path = `${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
  const { error } = await supabase.storage.from("chime-sounds").upload(path, file, {
    cacheControl: "3600",
    upsert: false,
  });
  if (error) throw error;
  const { data } = supabase.storage.from("chime-sounds").getPublicUrl(path);
  return data.publicUrl;
}

export async function updateRestaurantNotificationSoundUrl(restaurantId, url) {
  const { error } = await supabase
    .from("restaurants")
    .update({ notification_sound_url: url || null })
    .eq("id", restaurantId);
  if (error) throw error;
}
...
// Téléphone de la réservation honorée par cette commande sur place (via
// orders.reservation_id → reservations.customer_phone). null si la commande
// n'est liée à aucune réservation ou si celle-ci n'a pas de téléphone.
export async function fetchReservationPhoneForOrder(orderId) {
  const { data: ord, error: oErr } = await supabase
    ...
    .maybeSingle();
  if (oErr) throw oErr;
  const resId = ord?.reservation_id;
  ...
  const { data: res, error: rErr } = await supabase
    ...
    .maybeSingle();
  if (rErr) throw rErr;
  return res?.customer_phone || null;
}
...
export function useStaff() {
  ...
  return { staff: rows, loading, reload };
}

// Retente avec un nouveau PIN en cas de collision (index unique par
// établissement staff_pin_per_restaurant). Renvoie le PIN attribué.
export async function addStaffWithPin(fullName, contractType) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (!error) return pin;
    if (!error.message.includes("staff_pin_per_restaurant")) throw error;
  }
  throw new Error("Impossible d'attribuer un code PIN unique après plusieurs essais.");
}

// Renvoie le nouveau PIN attribué.
export async function resetStaffPin(id) {
  for (let attempt = 0; attempt < 5; attempt++) {
    const pin = generateStaffPin();
    const { error } = await supabase.from("staff").update({ pin_code: pin }).eq("id", id);
    if (!error) return pin;
    if (!error.message.includes("staff_pin_per_restaurant")) throw error;
  }
  throw new Error("Impossible d'attribuer un code PIN unique après plusieurs essais.");
...
export async function renameStaff(id, fullName) {
  const { error } = await supabase.from("staff").update({ full_name: String(fullName).trim() }).eq("id", id);
  if (error) throw error;
}

export async function setStaffContract(id, contractType) {
  const { error } = await supabase.from("staff").update({ contract_type: contractType }).eq("id", id);
  if (error) throw error;
}
...
// Aller-retour ponctuel (pas un hook temps réel) : la badgeuse ne s'en sert
// qu'à l'instant où le PIN vient d'être saisi.
export async function findStaffByPin(restaurantId, pin) {
  const { data, error } = await supabase
    .from("staff")
  ...
  if (error) throw error;
  if (!data) return null;
  return { id: data.id, fullName: data.full_name, restaurantId: data.restaurant_id };
}

// ------------------------------------------------------- pointage_entries --

// Dernier type de pointage enregistré pour ce salarié, tous jours confondus
// (la séquence arrivée → pause → fin de pause → départ reprend simplement
// où elle en était, même si le dernier pointage remonte à un jour ouvré
// précédent). null si le salarié n'a encore jamais pointé.
export async function getLastPointageType(staffId) {
  const { data, error } = await supabase
...
export async function addPointageEntry(staffId, restaurantId, type) {
  const { error } = await supabase.from("pointage_entries").insert({
    staff_id: staffId,
...
function mapPointageEntryRow(row) {
  return {
    id: row.id,
...
// Lignes d'une table restaurant-scopée depuis une borne locale donnée
// (recalculée à chaque reload), avec mise à jour realtime. Écrit à part de
// useRealtimeTable, qui ne supporte qu'un filtre eq simple et pas de borne
// de date en plus du restaurant. `getSince` renvoie un Date (minuit local
// du premier jour voulu) ; ex. aujourd'hui minuit, ou le lundi de la
// semaine en cours. Utilisé par pointage_entries (dateColumn
// "occurred_at") et pointage_corrections (dateColumn
// "corrected_occurred_at").
function useRowsSince({ table, dateColumn, mapRow, getSince }) {
  const eq = useRestaurantEq();
  ...
  const reload = useCallback(async () => {
    ...
    let query = supabase.from(table).select("*").gte(dateColumn, since.toISOString());
    if (eq?.column) query = query.eq(eq.column, eq.value);
    const { data, error } = await query;
    if (requestId !== latestRequestId.current) return;
    if (!error && data) setRows(data.map(mapRow));
    setLoading(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [table, dateColumn, eq?.column, eq?.value]);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`realtime:${table}_since:${instanceId}`)
...
export function useTodayPointageEntries() {
  const { rows, loading } = useRowsSince({
    table: "pointage_entries",
    dateColumn: "occurred_at",
    mapRow: mapPointageEntryRow,
    getSince: startOfTodayLocal,
  });
  return { entries: rows, loading };
}

// Depuis le lundi de la semaine calendaire en cours (heure locale) —
// utilisé pour le cumul hebdomadaire du dashboard de présence.
export function useThisWeekPointageEntries() {
  const { rows, loading } = useRowsSince({
    table: "pointage_entries",
    dateColumn: "occurred_at",
    mapRow: mapPointageEntryRow,
    getSince: startOfThisWeekLocal,
  });
  return { entries: rows, loading };
}

function mapPointageCorrectionRow(row) {
  return {
    id: row.id,
...
export function useTodayCorrections() {
  const { rows, loading } = useRowsSince({
    table: "pointage_corrections",
    dateColumn: "corrected_occurred_at",
    mapRow: mapPointageCorrectionRow,
    getSince: startOfTodayLocal,
  });
  return { corrections: rows, loading };
}

export function useThisWeekCorrections() {
  const { rows, loading } = useRowsSince({
    table: "pointage_corrections",
    dateColumn: "corrected_occurred_at",
    mapRow: mapPointageCorrectionRow,
...
export function useStaffShifts() {
  const { rows, loading, reload } = useRealtimeTable({
    table: "staff_shifts",
    mapRow: mapStaffShiftRow,
    eq: useRestaurantEq(),
  });
  return { staffShifts: rows, loading, reload };
}

export async function addStaffShift(staffId, weekday, startTime, endTime) {
...
// --------------------------------------------------------- export paie --
// Aller-retours ponctuels avec restaurant_id explicite (pas de hook temps
// réel, pas de useRestaurantEq) : même schéma que fetchDailySales /
// fetchConsumptionActuals, utilisés par les écrans Direction qui
// choisissent l'établissement via un sélecteur plutôt que via le
// RestaurantFilterContext.

export async function fetchStaffForRestaurant(restaurantId) {
  const { data, error } = await supabase
    .from("staff")
    .select("id, full_name, pin_code, contract_type, active")
    .eq("restaurant_id", restaurantId)
    .order("full_name");
  if (error) throw error;
...
// startDate/endDate : "YYYY-MM-DD", bornes locales (minuit inclus, minuit du
// jour après endDate exclu) — pas une comparaison de chaînes sur une colonne
// timestamptz, qui décalerait la frontière du jour selon le fuseau.
export async function fetchPointageEntriesForRange(restaurantId, startDate, endDate) {
  const [sy, sm, sd] = startDate.split("-").map(Number);
  const [ey, em, ed] = endDate.split("-").map(Number);
  const from = new Date(sy, sm - 1, sd, 0, 0, 0, 0);
  const to = new Date(ey, em - 1, ed + 1, 0, 0, 0, 0);
  const { data, error } = await supabase
    .from("pointage_entries")
    .select("id, staff_id, type, occurred_at")
    .eq("restaurant_id", restaurantId)
    .gte("occurred_at", from.toISOString())
    .lt("occurred_at", to.toISOString())
    .order("occurred_at");
  if (error) throw error;
  return (data || []).map(mapPointageEntryRow);
}

export async function fetchStaffShiftsForRestaurant(restaurantId) {
  const { data, error } = await supabase
    .from("staff_shifts")
    .select("id, staff_id, restaurant_id, weekday, start_time, end_time")
    .eq("restaurant_id", restaurantId);
  if (error) throw error;
  return (data || []).map(mapStaffShiftRow);
}

// ------------------------------------------------------ régularisations --

// Pointages d'un salarié pour une date locale donnée ("YYYY-MM-DD") — pour
// l'écran de régularisation (équipe). Pas de restaurant_id explicite : la
// RLS de l'espace équipe suffit à ne voir que son propre établissement.
export async function fetchPointageEntriesForStaffDay(staffId, date) {
  const [y, m, d] = date.split("-").map(Number);
  const from = new Date(y, m - 1, d, 0, 0, 0, 0);
  const to = new Date(y, m - 1, d + 1, 0, 0, 0, 0);
  const { data, error } = await supabase
    .from("pointage_entries")
    .select("id, staff_id, restaurant_id, type, occurred_at")
    .eq("staff_id", staffId)
    .gte("occurred_at", from.toISOString())
    .lt("occurred_at", to.toISOString())
    .order("occurred_at");
...
export async function fetchCorrectionsForStaffDay(staffId, date) {
  const [y, m, d] = date.split("-").map(Number);
  const from = new Date(y, m - 1, d, 0, 0, 0, 0);
  const to = new Date(y, m - 1, d + 1, 0, 0, 0, 0);
  const { data, error } = await supabase
    .from("pointage_corrections")
    .select("id, original_entry_id, staff_id, restaurant_id, type, corrected_by, reason, corrected_occurred_at")
    .eq("staff_id", staffId)
    .gte("corrected_occurred_at", from.toISOString())
    .lt("corrected_occurred_at", to.toISOString())
    .order("corrected_occurred_at");
  if (error) throw error;
  return (data || []).map(mapPointageCorrectionRow);
}

// originalEntryId : null si la régularisation ajoute un pointage manquant
// plutôt que de corriger l'heure d'un pointage existant.
export async function addPointageCorrection({
  staffId,
  restaurantId,
  type,
  originalEntryId,
  ...
  const { error } = await supabase.from("pointage_corrections").insert({
    staff_id: staffId,
    restaurant_id: restaurantId,
    type,
    original_entry_id: originalEntryId || null,
    corrected_occurred_at: correctedOccurredAt,
...
// Pour l'export paie (Direction, restaurant_id explicite — pas de RLS
// implicite puisque le compte manager n'a pas d'établissement propre).
export async function fetchPointageCorrectionsForRange(restaurantId, startDate, endDate) {
  const [sy, sm, sd] = startDate.split("-").map(Number);
  const [ey, em, ed] = endDate.split("-").map(Number);
  const from = new Date(sy, sm - 1, sd, 0, 0, 0, 0);
  const to = new Date(ey, em - 1, ed + 1, 0, 0, 0, 0);
