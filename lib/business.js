...
export function remainingForSlot(orders, slot) {
  const used = orders
    ...
    .flatMap((o) => {
      if (o.slotAllocations && o.slotAllocations.length > 0) return o.slotAllocations;
      // Commande programmée la veille (ou plus tôt) pour un créneau précis :
      // elle n'a jamais réservé de vraies places (les créneaux du jour visé
      // n'existaient pas encore à sa création — voir ScheduledOrderFlow), mais
      // dès que son jour arrive, elle doit décompter la capacité du créneau
      // qu'elle vise, comme n'importe quelle autre commande.
      if (o.scheduledTime === slot.label && isOrderActiveToday(o) && o.pizzaCount > 0) {
        return [{ slotId: slot.id, qty: o.pizzaCount }];
      }
      return [];
    })
...
export function allUpcomingSlotsForStaff(orders, slots, needed) {
  ...
  const nowMinutes = now.getHours() * 60 + now.getMinutes();
  return slots
    .map((s) => ({ id: s.id, label: s.label, mins: parseMinutes(s.label), remaining: remainingForSlot(orders, s) }))
