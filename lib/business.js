...
// Ordre d'affichage des articles sur les écrans Service/Caisse — boissons
// d'abord (pour que le service à table sache quoi prendre en premier), puis
// antipasti, puis les plats (pizzas/panuzzo/salades), puis les desserts.
const ITEM_DISPLAY_ORDER = ["boisson", "biere", "vin", "cocktail", "antipasti", "pizza", "panuzzo", "salade", "supplement", "sans", "dessert"];
export function sortItemsForDisplay(items) {
  return [...(items || [])].sort((a, b) => {
    const ai = ITEM_DISPLAY_ORDER.indexOf(a.cat);
    const bi = ITEM_DISPLAY_ORDER.indexOf(b.cat);
    return (ai === -1 ? 999 : ai) - (bi === -1 ? 999 : bi);
  });
}

// File du four : les commandes à emporter d'abord, puis le sur place en
...
export function sortKitchenQueue(list) {
  return [...list].sort((a, b) => {
    const aTakeaway = a.serviceType === TAKEAWAY_SERVICE_TYPE;
    const bTakeaway = b.serviceType === TAKEAWAY_SERVICE_TYPE;
