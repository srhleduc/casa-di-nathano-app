...
// retenir l'horaire souhaité). Dès que son jour arrive, on lui reconstitue la
// répartition qu'elle aurait obtenue si elle avait été prise aujourd'hui en
// visant ce créneau : comme buildPlans le ferait en partant du plus tôt, mais
// à rebours depuis l'horaire demandé (qui doit rester le dernier créneau de
// la série), en remplissant la capacité encore libre de chaque créneau
// précédent jusqu'à couvrir toutes les pizzas. Recalculée à chaque appel de
// remainingForSlot (une par créneau) — coût négligeable vu le volume de
// commandes d'une soirée, pas besoin de mise en cache.
function virtualScheduledAllocations(orders, allSlots) {
  ...
  const realUsed = new Map();
  active
    .flatMap((o) => o.slotAllocations || [])
    .forEach((a) => realUsed.set(a.slotId, (realUsed.get(a.slotId) || 0) + a.qty));
  ...
  for (const o of scheduledPending) {
    const targetIdx = sortedSlots.findIndex((s) => s.label === o.scheduledTime);
    if (targetIdx === -1) continue; // pas de créneau du jour à cet horaire, rien à reconstituer
    let left = o.pizzaCount;
    for (let idx = targetIdx; idx >= 0 && left > 0; idx--) {
      const slot = sortedSlots[idx];
      const usedHere = (realUsed.get(slot.id) || 0) + virtual.filter((v) => v.slotId === slot.id).reduce((s, v) => s + v.qty, 0);
      const remaining = slot.capacity - usedHere;
      if (remaining <= 0) continue;
      virtual.push({ orderId: o.id, slotId: slot.id, label: slot.label, qty: Math.min(remaining, left) });
      left -= Math.min(remaining, left);
    }
    // Si la capacité cumulée de tous les créneaux jusqu'au début du service
    // ne suffit toujours pas, le reliquat n'est simplement pas compté — comme
    // pour toute commande dont la taille dépasse ce que le four peut absorber.
  }
  return virtual;
}
...
export function computeScheduledOrderSlotAllocations(orders, allSlots) {
  ...
  // Reconstitué à rebours (créneau visé d'abord), remis ici dans l'ordre
  // chronologique — comme buildPlans le produirait — pour un affichage
  // identique à celui d'une grosse commande répartie prise le jour même.
  return Array.from(byOrder, ([orderId, slotAllocations]) => ({
    orderId,
    slotAllocations: [...slotAllocations].sort((a, b) => (parseMinutes(a.label) ?? 0) - (parseMinutes(b.label) ?? 0)),
  }));
}
...
export function remainingForSlot(orders, slot, allSlots) {
  const used = orders
    ...
    .flatMap((o) => o.slotAllocations || [])
    .concat(virtualScheduledAllocations(orders, allSlots))
