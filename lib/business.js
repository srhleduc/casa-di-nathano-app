...
import { toMin, mondayOf } from "./reservation/services";
...
// Téléphone client pour l'engagement sur les commandes en ligne — on stocke le
// numéro brut normalisé (espaces/points/tirets retirés), sans aucun
// rapprochement avec un profil existant. Validation volontairement souple :
// format français à 10 chiffres (0X…) ou international +33X…
export function normalizePhoneFr(raw) {
  return (raw || "").replace(/[\s.\-()]/g, "");
}
export function isValidPhoneFr(raw) {
  const s = normalizePhoneFr(raw);
  return /^0[1-9]\d{8}$/.test(s) || /^\+33[1-9]\d{8}$/.test(s);
}

// Forme canonique UNIQUE utilisée comme clé du programme de fidélité :
// 10 chiffres 0XXXXXXXXX (le +33.../0033... est ramené au 0 domestique).
// Renvoie null si ce n'est pas un numéro français valide — l'appelant doit
// alors refuser l'opération plutôt que de créer un client sur une clé douteuse.
// La fonction Postgres award_loyalty_points re-normalise de la même façon, car
// le click & collect arrive par la base et la caisse par le client.
export function canonicalLoyaltyPhone(raw) {
  let s = normalizePhoneFr(raw);
  if (s.startsWith("+33")) s = "0" + s.slice(3);
  else if (s.startsWith("0033")) s = "0" + s.slice(4);
  // Outre-mer : +590 (Guadeloupe), +594 (Guyane) — indicatif à 3 chiffres,
  // le numéro national reste 0 + 9 chiffres comme en métropole.
...
// Desserts encore commandables pour un client en click & collect à emporter
// (jamais staffMode, jamais sur place — pas de logique de dépannage entre
// formats ici, voir OrderScreen.dessertAvailability pour ce cas côté équipe).
// Utilisé par le popup "un petit dessert ?" proposé à la validation de la
// commande (voir DessertUpsellModal) et par TakeawayOrder pour savoir s'il y
// a quelque chose à proposer avant de l'afficher. `requiredOptionsUnavailable`
// est injecté par l'appelant (dépend de useOptionGroups, un hook React).
export function availableTakeawayDesserts(menu, groups, { ruptures, dessertStock, orders, activeServiceGroups, requiredOptionsUnavailable }) {
  return (menu || []).filter((m) => {
    if (m.cat !== "dessert") return false;
    if (m.staffOnly || m.dineInOnly) return false;
    if (m.serviceRestriction && activeServiceGroups && !activeServiceGroups.has(m.serviceRestriction)) return false;
    if ((ruptures || []).includes(m.id)) return false;
    if (requiredOptionsUnavailable && requiredOptionsUnavailable(m)) return false;
    const group = dessertStockGroupFor(groups, m.name, true);
    if (group && !group.unlimited && remainingForDessertGroup(orders || [], dessertStock || {}, group) <= 0) return false;
    return true;
  });
}
...
// pastille commande (satAdditionAt) ET chaque ligne concernée (it.satNew).
// Tant qu'il reste quelque chose de non pointé, la commande est signalée —
// sur l'écran Service comme sur le plan des Réservations.
export function hasUnseenSatAddition(order) {
  return Boolean(order?.satAdditionAt) || (order?.items || []).some((it) => it.satNew);
}
...
function backwardFillPlan(orders, allSlots, targetLabel, needed) {
  ...
  // Si la capacité cumulée jusqu'au début du service ne suffit toujours pas,
  // le reliquat déborde sur le créneau visé lui-même plutôt que de rester
  // sans répartition — le client vient à cette heure-là de toute façon, le
  // four devra suivre. Sans ce débordement, l'appelant (voir le garde
  // "total !== pizzaCount" dans TeamSpace) refuse d'écrire un plan partiel et
  // la commande reste affichée avec son seul horaire brut, sans qu'aucune
  // répartition ni alerte ne signale le manque de place à l'équipe.
  if (left > 0) {
    const target = sortedSlots[targetIdx];
    const existing = plan.find((p) => p.slotId === target.id);
    if (existing) existing.qty += left;
    else plan.push({ slotId: target.id, label: target.label, qty: left });
  }
...
// Résumé lisible des horaires d'ouverture click & collect d'un jour donné
// (ligne takeaway_hours), ex. "Midi 12:00-14:00 · Soir 18:00-21:30", "Soir
// 18:00-21:30" (un seul service actif ce jour-là), ou null si fermé ce
// jour-là (les deux fenêtres vides). Sert à la fois au bandeau "aujourd'hui"
// (WelcomeScreen) et à la liste hebdomadaire de l'écran "fermé aujourd'hui"
// (voir TakeawayOrder.jsx).
export function describeTakeawayHoursToday(hoursToday) {
  if (!hoursToday) return null;
  const parts = [];
  if (hoursToday.midiOpen && hoursToday.midiClose) parts.push(`Midi ${hoursToday.midiOpen}-${hoursToday.midiClose}`);
  if (hoursToday.soirOpen && hoursToday.soirClose) parts.push(`Soir ${hoursToday.soirOpen}-${hoursToday.soirClose}`);
  return parts.length ? parts.join(" · ") : null;
}

// Libellés des 7 jours de la semaine (0=dimanche…6=samedi), dans l'ordre
// d'affichage français (lundi → dimanche).
...
// Fermeture globale ("toute la pizzeria", service_exceptions.serviceNumber
// null) active pour une date — priorité à l'exception la plus étroite,
// 'off' gagnant sur 'on' en cas d'égalité. Même règle de résolution que
// servicesForDate (lib/reservation/services.js), restreinte aux exceptions
// globales — utilisée pour le bandeau "fermé aujourd'hui" du click & collect
// (voir TakeawayOrder.jsx) et dupliquée (pas d'import inter-runtime possible)
// dans supabase/functions/generate-daily-slots pour la génération des créneaux.
export function isClosedByGlobalException(dateStr, exceptions) {
  const covering = (exceptions || []).filter((e) => e.serviceNumber == null && e.dateStart <= dateStr && dateStr <= e.dateEnd);
  const spanDays = (e) => (new Date(e.dateEnd) - new Date(e.dateStart)) / 86400000;
  const sorted = covering.sort((a, b) => spanDays(a) - spanDays(b) || (a.mode === "off" ? -1 : 1) - (b.mode === "off" ? -1 : 1));
  return sorted[0]?.mode === "off";
}

// Vrai si le click & collect n'a aucune fenêtre d'ouverture aujourd'hui —
// ni horaire midi/soir configuré (takeawayHours du jour de semaine), ni
// fermeture exceptionnelle. Sert à avertir le client AVANT qu'il ne compose
// sa commande (voir TakeawayOrder.jsx), plutôt que de le laisser découvrir
// l'absence de créneau seulement au moment de valider.
export function isTakeawayClosedToday(dateStr, hoursToday, exceptions) {
  if (isClosedByGlobalException(dateStr, exceptions)) return true;
  return !hoursToday || (!hoursToday.midiOpen && !hoursToday.soirOpen);
}
...
// Transforme un `slotChoice` (résultat de computeSlotOptions) en liste plate
// d'options affichables — un `key` stable par option permet de vérifier
// qu'un choix fait à un instant T (ex. sur SlotScreen) est toujours valable
// à un instant T+n (voir isSlotChoiceStillOffered, utilisé juste avant la
// validation finale côté client pour éviter de confirmer un créneau devenu
// trop proche ou complet entre-temps). Partagé avec SlotScreen.jsx pour ne
// jamais dupliquer cette dérivation mode "single"/"split".
export function deriveSlotDisplayOptions(slotChoice, pizzaCount) {
  const mode = slotChoice?.mode;
  if (mode === "single") {
    return (slotChoice.options || []).map((s) => ({
      key: s.id,
      label: s.label,
  ...
  if (mode === "split") {
    return (slotChoice.plans || []).map((p, i) => ({
      key: `plan-${i}`,
...
// Un créneau choisi par le client sur SlotScreen peut devenir invalide s'il
// reste plusieurs minutes sur cet écran avant de valider (le créneau passe
// sous la marge de sécurité, ou se remplit via d'autres commandes) : rien ne
// rafraîchit `selectedOption` tant qu'il est déjà défini. Recalcule les
// options à l'instant présent et vérifie que le choix figure toujours dedans
// — à appeler juste avant la soumission finale (voir TakeawayOrder.jsx).
export function isSlotChoiceStillOffered(orders, slots, pizzaCount, marginMinutes, selectedOption) {
  const freshChoice = computeSlotOptions(orders, slots, pizzaCount, marginMinutes);
  if (!selectedOption) return { valid: true, freshChoice };
  const freshOptions = deriveSlotDisplayOptions(freshChoice, pizzaCount);
  return { valid: freshOptions.some((o) => o.key === selectedOption.key), freshChoice };
}
...
// Catégories qui passent par le four et/ou la Finition (les boissons/desserts
// se gèrent au niveau de l'article, pas besoin de ce recalcul). Sert à décider
// si un ajout tardif sur une commande déjà avancée doit rouvrir le circuit
// cuisine — utilisé par EditOrderModal.save(), StaffOrderFlow et /sat (le
// résultat > 0 alimente p_reopen_kitchen de sat_append_items).
export const KITCHEN_PIPELINE_CATS = ["pizza", "panuzzo", "supplement", "sans", "antipasti", "salade"];
export function kitchenPendingQty(items) {
  return (items || [])
    .filter((i) => !i.served && i.phase !== "apero" && KITCHEN_PIPELINE_CATS.includes(i.cat))
    .reduce((s, i) => s + i.qty, 0);
}

// Libellé d'affichage d'une commande sur place à partir de son rattachement aux
// tables : "Table 12 + 13" si des tables enregistrées sont cochées, sinon le
// texte libre, sinon un repli neutre. Réutilise le tri naturel de sortByTableName.
// Nom affiché d'une table : le libellé libre saisi par l'équipe (ex. « T1 »,
// « E3 »), ou à défaut son code QR.
export function tableDisplayName(t) {
  if (!t) return "";
  return (t.label || "").trim() || t.number || "";
}

// Ordre de remplissage des tables pour le moteur de réservation : d'abord le
// champ priority_order (glisser-déposer de l'écran « Ordre de priorité »),
// puis, à défaut ou à égalité, le tri naturel du nom (T1 avant T9). Source de
// vérité unique partagée par TablePriorityList et les appels au solveur, pour
// que le moteur reçoive les tables déjà dans l'ordre voulu même si aucun
// priority_order n'a encore été enregistré.
const NO_PRIORITY = Number.MAX_SAFE_INTEGER;
export function fillPriorityRank(t) {
  return t && t.priorityOrder != null ? Number(t.priorityOrder) : NO_PRIORITY;
}
export function sortByFillPriority(list) {
  return [...(list || [])].sort(
    (a, b) => fillPriorityRank(a) - fillPriorityRank(b) || tableNameCollator.compare(tableDisplayName(a), tableDisplayName(b))
  );
}

// Ordre des combinaisons de tables pour le moteur : d'abord les « habituelles »
// (is_usual), puis pénalité croissante, puis une clé stable (ids triés) — le
// moteur suit cet ordre à choix équivalent, et donne toujours la priorité à une
// combinaison habituelle disponible sur une exceptionnelle.
export function sortByComboPriority(list) {
  return [...(list || [])].sort(
    (a, b) =>
      (a.isUsual === false ? 1 : 0) - (b.isUsual === false ? 1 : 0) ||
      (Number(a.penaltyScore) || 0) - (Number(b.penaltyScore) || 0) ||
      [...(a.tableIds || [])].sort().join("|").localeCompare([...(b.tableIds || [])].sort().join("|"))
  );
}

export function tableDisplayLabel({ tableIds, tableLabel } = {}, tables = []) {
  const names = (tableIds || [])
    .map((id) => tableDisplayName(tables.find((t) => t.id === id)))
    .filter(Boolean)
    .sort((a, b) => tableNameCollator.compare(a, b));
  if (names.length > 0) return names.join(" + ");
  const free = (tableLabel || "").trim();
  return free || "Commande équipe";
}

// Répartition des lignes/commandes par source de saisie (reporting SAT §7) —
// même découpage que l'archive nocturne source_stats_daily, pour la vue "en
// direct" du jour. `source` absente/null = serveuse.
export function sourceBreakdown(orders) {
  const b = { linesTotal: 0, linesServeuse: 0, linesSat: 0, linesCC: 0, ordersTotal: 0, ordersAuto: 0 };
  for (const o of orders || []) {
    ...
    for (const it of o.items || []) {
      ...
      if (it.source === "sat") {
        b.linesSat += 1;
        auto = true;
      } else if (it.source === "click_and_collect") {
        b.linesCC += 1;
        auto = true;
      } else {
        b.linesServeuse += 1;
    ...
    if (auto) b.ordersAuto += 1;
  }
  return b;
}

// Commande sur place "ouverte" (pas encore encaissée) rattachée à l'une des
// tables visées — pour y ajouter les nouveaux articles au lieu de créer une
// commande concurrente (une seule commande active par table). Le texte libre
// ne matche que par égalité exacte (jamais de rattachement via le QR /sat).
export function findOpenDineInOrderForTables(orders, { tableIds, tableLabel } = {}) {
  const ids = tableIds || [];
  const free = (tableLabel || "").trim().toLowerCase();
  if (ids.length === 0 && !free) return null;
  return (
    (orders || []).find((o) => {
      if (isTakeawayLike(o.serviceType)) return false;
      if (o.status === "servie" || isOrderPaid(o) || !isOrderActiveToday(o)) return false;
      const oIds = o.tableIds || [];
      if (ids.length > 0 && oIds.some((x) => ids.includes(x))) return true;
      if (free && (o.tableLabel || "").trim().toLowerCase() === free) return true;
      return false;
    }) || null
  );
}
...
// Minutes travaillées par salarié et par jour calendaire (heure locale), à
// partir d'une liste de pointage_entries (voir lib/data.js). Une arrivée
// encore ouverte (pas de "depart") ne compte que jusqu'à `asOf` si fourni
// (vue temps réel : le salarié est encore en poste) ; sans `asOf` (export
// paie historique), elle ne compte pas — pas d'heure devinée au-delà du
// dernier pointage connu. Renvoie une Map "staffId|YYYY-MM-DD" -> minutes.
export function computeDailyWorkedMinutes(entries, asOf) {
  const byStaffDay = new Map();
  const sorted = [...entries].sort((a, b) => new Date(a.occurredAt) - new Date(b.occurredAt));

  const openArrival = new Map(); // staffId -> Date
  const openPause = new Map(); // staffId -> Date

  function dayKey(staffId, date) {
    const d = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
    return `${staffId}|${d}`;
  }
  function addMinutes(staffId, date, delta) {
    const key = dayKey(staffId, date);
    byStaffDay.set(key, (byStaffDay.get(key) || 0) + delta);
  }

  for (const e of sorted) {
    const at = new Date(e.occurredAt);
    if (e.type === "arrivee") {
      openArrival.set(e.staffId, at);
    } else if (e.type === "depart") {
      const start = openArrival.get(e.staffId);
      if (start) {
        addMinutes(e.staffId, start, (at - start) / 60000);
        openArrival.delete(e.staffId);
      }
    } else if (e.type === "pause_debut") {
      openPause.set(e.staffId, at);
    } else if (e.type === "pause_fin") {
      const start = openPause.get(e.staffId);
      if (start) {
        addMinutes(e.staffId, start, -(at - start) / 60000);
        openPause.delete(e.staffId);
      }
    }
  }

  if (asOf) {
    for (const [staffId, start] of openArrival.entries()) {
      addMinutes(staffId, start, (asOf - start) / 60000);
    }
  }
...
// Regroupe une Map "staffId|YYYY-MM-DD" -> minutes (computeDailyWorkedMinutes)
// par semaine calendaire française (lundi → dimanche). Renvoie une Map
// "staffId|lundiDeLaSemaine" -> minutes travaillées cette semaine-là.
export function groupWorkedMinutesByWeek(dailyMinutes) {
  const byWeek = new Map();
  for (const [key, minutes] of dailyMinutes.entries()) {
    const [staffId, date] = key.split("|");
    const weekKey = `${staffId}|${mondayOf(date)}`;
    byWeek.set(weekKey, (byWeek.get(weekKey) || 0) + minutes);
  }
  return byWeek;
}

// Volume hebdomadaire prévu pour un salarié d'après son planning récurrent
// (staff_shifts) — indépendant de la semaine calendaire puisque le planning
// se répète à l'identique chaque semaine. 0 si aucun créneau défini (pas de
// base de comparaison, pas un dépassement à 0h).
export function weeklyPlannedMinutesForStaff(staffShifts, staffId) {
  return staffShifts
    .filter((s) => s.staffId === staffId)
    .reduce((sum, s) => sum + (toMin(s.endTime) - toMin(s.startTime)), 0);
}

// Fusionne les pointages bruts (immuables) avec les régularisations
// (pointage_corrections) en une liste "effective" à utiliser pour tout
// calcul d'heures — jamais pour ré-écrire les pointages en base (voir
// art. L.3171-4 : le pointage réel reste la preuve, la régularisation est
// tracée à part). Une régularisation avec `originalEntryId` remplace
// l'heure d'un pointage existant ; sans lui, elle ajoute un pointage
// manquant (ex. oubli de départ). Chaque entrée effective issue d'une
// régularisation porte `corrected: true` pour l'affichage.
//
// Si `originalEntryId` est renseigné mais que ce pointage n'est pas dans
// `entries` (ex. régularisation qui change aussi le jour, hors de la
// fenêtre chargée), on l'ignore plutôt que de l'ajouter en double —
// mieux vaut manquer une correction que compter deux fois le même
// pointage.
export function applyCorrections(entries, corrections) {
  const byId = new Map(entries.map((e) => [e.id, { ...e }]));
  const additions = [];

  for (const c of corrections) {
    if (c.originalEntryId) {
      const e = byId.get(c.originalEntryId);
      if (e) {
        e.occurredAt = c.correctedOccurredAt;
        e.corrected = true;
      }
    } else {
      additions.push({
        ...
        type: c.type,
        occurredAt: c.correctedOccurredAt,
        corrected: true,
      });
    }
  }

  return [...byId.values(), ...additions];
}
