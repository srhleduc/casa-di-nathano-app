...
-- =====================================================================
-- GOOGLE WALLET — carte de fidélité dématérialisée (Loyalty pass).
--
-- Additif pur : ne touche NI award_loyalty_points, NI les triggers paliers,
-- NI le calcul du solde. On expose seulement, via des fonctions security
-- definer appelables avec la clé anon (les routes API n'ont pas de session) :
--   - lecture ciblée des infos nécessaires au pass (nom + solde) ;
--   - marquage « pass ajouté » (optimiste au 1er lien généré, le webhook
--     Google confirmera/écrasera plus tard).
...
-- =====================================================================
-- PAGE CLIENT PUBLIQUE /mon-compte — le client consulte son solde et
-- ajoute sa carte Google Wallet lui-même, sans passer par la pizzeria.
-- Vérification légère (téléphone + nom de famille), pas de vraie auth :
-- l'enjeu est bas (des points de fidélité). Anti-brute-force : rate limit
-- par IP côté base (table loyalty_public_lookups), et réponse identique
-- pour "numéro inconnu" et "nom incorrect" (pas d'oracle d'énumération).
-- =====================================================================
...
-- Résout un client par téléphone + nom de famille pour la page publique.
-- p_nom = nom de famille saisi : match "mot entier" insensible casse/accents
-- dans la colonne nom (souvent "Prénom NOM").
-- Rate limit : seulement si l'appelant est identifiable (p_ip ressemble à une
-- IP). Dans ce cas 20 tentatives / IP / 10 min. Si l'IP n'est pas exploitable
-- ('unknown'), on NE bucketise PAS tous les clients ensemble : garde-fou
-- global très large (200 req 'unknown' / 10 min) uniquement contre un flood.
-- status : 'ok' | 'not_found' | 'rate_limited'. 'not_found' couvre aussi bien
-- "téléphone inconnu" que "nom incorrect" (aucun indice).
...
-- (Repris dans supabase/migrations_manual/reservations.sql.)
-- Moteur de réservation + optimisation de salle. Base unique partagée par
-- les 3 déploiements : cloisonnement par restaurant_id text + RLS. La table
...
-- Zone ouverte / fermée (interrupteur rapide, ex. terrasse fermée s'il pleut).
